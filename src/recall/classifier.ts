...
async function classifyWithAI(text: string, settings: RecallSettings): Promise<FilterDecision> {
  ...
  try {
          ...
          json_schema: {
            ...
            schema: {
              ...
              properties: {
                category: {
                  type: 'string',
                  enum: ['keep', 'politics', 'polarizing', 'off-topic', 'custom'],
                },
    ...
    if (
      ...
      !['keep', 'politics', 'polarizing', 'off-topic', 'custom'].includes(category)
    )
      return keepOnError();
    const allowed =
      category === 'custom' ||
